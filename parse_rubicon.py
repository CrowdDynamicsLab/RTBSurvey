...
crawl_data_dir = 'crawl_data/conservative/crawl-data.sqlite'
...
for row in rows:
    if "fastlane" in row[3]:
        ...
        auctions_list.append(auction.parse_response_rubicon(row=row, response = response,price_var = '/"cpm/"', cid_var = '/"creative_id/"' , dim_vars = [';width','/"height'], adname_var = '/"advbrand/"', adid_var = '/"ad_id/"', val_offset = 1, str_offset=3))
